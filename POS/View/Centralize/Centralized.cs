...
namespace POS
{
    public partial class Centralized : Form
    {
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            int indexMonth = -2;
            var iDuration = entity.Settings.Where(x => x.Key == "CentralizeDuration").FirstOrDefault();
            if (iDuration != null && !string.IsNullOrEmpty(iDuration.Value))
            {
                indexMonth = int.Parse(iDuration.Value);
            }
            DateTime today = DateTime.Today;
            DateTime TranDate = DateTime.Today.AddMonths(indexMonth);


            //string dtToday = today.ToString("yyyy-MM-dd");
            string dtToday = "2024-04-09";

            //string dtTranDate = TranDate.ToString("yyyy-MM-dd");
            string dtTranDate = "2024-04-01";


            connetionString = ConfigurationManager.ConnectionStrings["MyConnectionString"].ConnectionString;

            connection = new SqlConnection(connetionString);
            //string sqlProduct4Tax = "select P.ID,ISNULL(P.Name,'') AS Name,ProductCode,BarCode,Price,ISNULL(Qty,0) AS Qty,ISNULL(BrandId,0) As BrandId,ISNULL(ProductLocation,'') As ProductLocation,ISNULL(P.ProductCategoryId,0) As ProductCategoryId,ISNULL(ProductSubCategoryId,0) As ProductSubCategoryId,ISNULL(UnitId,0) As UnitId,ISNULL(TaxId,0) As TaxId,ISNULL(MinStockQty,0) As MinStockQty,ISNULL(DiscountRate,0.00) As DiscountRate,ISNULL(IsWrapper,'false') As IsWrapper,ISNULL(IsConsignment,'false') As IsConsignment,ISNULL(IsDiscontinue,'false') As IsDiscontinue,ISNULL(IsPromotionProduct,'false') As IsPromotionProduct,ISNULL(IsNovelty,0) As IsNovelty,ISNULL(ConsignmentPrice,0) As ConsignmentPrice,ISNULL(ConsignmentCounterId,0) As ConsignmentCounterId,ISNULL(Size,'') As Size,ISNULL(PurchasePrice,0) As PurchasePrice,ISNULL(IsNotifyMinStock,'false') As IsNotifyMinStock,UpdateDate as UpdateDate, B.Name as BName, PC.Name as CName, ISNULL(PS.Name,'') as SCName, ISNuLL(T.Name, '') as TaxName from Product as P left join Brand as B on P.BrandId = B.Id left join ProductCategory as PC on P.ProductCategoryId = PC.Id left join ProductSubCategory as PS on P.ProductSubCategoryId = PS.Id left join Tax as T on T.Id = P.TaxId";

            string sqlProduct = "select P.ID,ISNULL(P.Name,'') AS Name,ProductCode,BarCode,Price,ISNULL(Qty,0) AS Qty,ISNULL(BrandId,0) As BrandId,ISNULL(lineId,0) As lineID,ISNULL(ProductLocation,'') As ProductLocation,ISNULL(P.ProductCategoryId,0) As ProductCategoryId,ISNULL(ProductSubCategoryId,0)As ProductSubCategoryId,ISNULL(UnitId,0) As UnitId,ISNULL(TaxId,0) As TaxId,ISNULL(MinStockQty,0) As MinStockQty,ISNULL(DiscountRate,0.00) As DiscountRate,ISNULL(IsWrapper,'false') As IsWrapper,ISNULL(IsConsignment,'false') As IsConsignment,ISNULL(IsDiscontinue,'false') As IsDiscontinue,ISNULL(IsPromotionProduct,'false') As IsPromotionProduct,ISNULL(IsNovelty,0) As IsNovelty,ISNULL(ConsignmentPrice,0) As ConsignmentPrice,ISNULL(ConsignmentCounterId,0) As ConsignmentCounterId,ISNULL(Size,'') As Size,ISNULL(PurchasePrice,0) As PurchasePrice,ISNULL(IsNotifyMinStock,'false') As IsNotifyMinStock,UpdateDate as UpdateDate, B.Name as BName, L.Name as LName,PC.Name as CName, ISNULL(PS.Name,'') as SCName, ISNuLL(T.Name, '') as TaxName from Product as P left join Brand as B on P.BrandId = B.Id left join Line as L on P.LineID = L.Id left join ProductCategory as PC on P.ProductCategoryId = PC.Id left join ProductSubCategory as PS on P.ProductSubCategoryId = PS.Id left join Tax as T on T.Id = P.TaxId";
            // and P.Id in (select distinct TD.ProductId as ProductId from TransactionDetail as TD inner join [Transaction] as t on t.Id=TD.TransactionId where t.IsComplete=1 and cast(t.UpdatedDate as date) >= Cast( '"+dtTranDate+"' as date) and cast(t.UpdatedDate as date) <= Cast('"+dtToday+"' as date) UNION (select distinct MustIncludeProductId as ProductId from GiftSystem where MustIncludeProductId is not null) UNION (select distinct GiftProductId as ProductId from GiftSystem where GiftProductId is not null) UNION select distinct ProductId as ProductId from ProductInNovelty UNION select distinct ParentProductID as ProductId from SPDetail UNION select distinct ChildProductID as ProductId from SPDetail)";
            string sqlCustomer = "select Cus.*, C.CityName from Customer as Cus left join City as C on Cus.CityId = C.Id where Cus.Id in ((select distinct CustomerId from [Transaction] ts where ts.[DateTime]>='"+dtTranDate+ "' and ts.IsComplete=1) UNION (select distinct CustomerId from GiftCard) UNION (select distinct CustomerId from Loc_PointRedeemHistory pr where pr.[DateTime]>='"+dtTranDate+ "') UNION (select distinct CustomerId from CustomerLevelRevokeHistory clrh where clrh.ActionOn>='"+dtTranDate+"'))";
            string sqlTransaction = "select T.ID,T.DateTime,T.UpdatedDate,T.UserId,T.CounterId,T.Type,T.IsPaid,T.IsComplete,T.IsActive,T.IsDeleted,T.Loc_IsCalculatePoint,T.PaymentTypeId,T.TaxAmount,T.DiscountAmount,T.TotalAmount,T.RecieveAmount,ISNULL(T.ParentId,'') As ParentID,T.GiftCardId,T.CustomerId,T.ReceivedCurrencyId,T.ShopId, C.Address, C.Email, C.PhoneNumber, C.CustomerCode, Co.Name as CounterName, U.Name as UName, S.ShopName as ShopName, S.Address as SAddress,T.IsExported from [Transaction] as T inner join Customer as C on T.CustomerId = C.Id inner join Counter Co on Co.Id = T.CounterId inner join [User] as U on T.UserId = U.Id left join Shop as S on S.Id = T.ShopId  WHERE T.IsComplete=1 and cast(T.UpdatedDate as date) >= Cast('" + dtTranDate + "' as date) and cast(T.UpdatedDate as date) <= Cast('" + dtToday + "' as date) ";
            ...
            string sqlLine = "Select * from Line";
            ...
            // string SqlSetting = "Select * from Setting"; // before SAP
            ...
            string SqlGiftCard = "Select distinct G.*, C.Address, C.Email, C.PhoneNumber, C.CustomerCode from GiftCard as G inner join Customer C on G.CustomerId = C.Id inner join GiftCardInTransaction as GC on GC.GiftCardId = G.Id inner join [Transaction] as T on T.Id = GC.TransactionId where ((Cast(T.UpdatedDate as date) >= Cast('" + dtTranDate + "' as date) and Cast(T.UpdatedDate as date) <= Cast('" + dtToday + "' as date)) OR Cast(G.IsUsedDate as date) >= Cast('" + dtTranDate + "' as date) and cast(G.IsUsedDate as date) <= Cast('" + dtToday + "' as date)) and G.IsDeleted = 0";
            ...
            //string SqlGiftSystem = "select G.Id,G.Type,G.Name,G.MustBuyCostFrom,G.MustBuyCostTo,G.MustIncludeProductId as MustIncludeProductId ,G.FilterBrandId as FilterBrandId,G.FilterCategoryId as FilterCategoryId, G.FilterSubCategoryId as FilterSubCategoryId  ,G.ValidFrom,G.ValidTo,G.UsePromotionQty,G.PromotionQty,G.GiftProductId,G.PriceForGiftProduct,G.GiftCashAmount,G.DiscountPercentForTransaction,G.UseBrandFilter,G.UseCategoryFilter,G.UseSubCategoryFilter,G.UseProductFilter,G.IsActive,G.UseSizeFilter,G.UseQtyFilter,G.FilterSize,G.FilterQty,p1.ProductCode as MustProductCode, p2.ProductCode as GiftProductCode, ISNULL(PC.Name,'') as CName, ISNULL(PS.Name,'') as SCName  from GiftSystem as G left join Product as P1 on P1.Id = G.MustIncludeProductId left join Product as P2 on p2.Id = G.GiftProductId left join ProductCategory as PC on G.FilterCategoryId = PC.Id left join ProductSubCategory as PS on G.FilterSubCategoryId = PS.Id";
            string SqlGiftSystem = "select G.Id,G.Type,G.Name,G.MustBuyCostFrom,G.MustBuyCostTo,G.UseLineFilter,G.MustIncludeProductId as MustIncludeProductId ,G.FilterBrandId as FilterBrandId,G.FilterLineId,G.FilterCategoryId as FilterCategoryId, G.FilterSubCategoryId as FilterSubCategoryId  ,G.ValidFrom,G.ValidTo,G.UsePromotionQty,G.PromotionQty,G.GiftProductId,G.PriceForGiftProduct,G.GiftCashAmount,G.DiscountPercentForTransaction,G.UseBrandFilter,G.UseCategoryFilter,G.UseSubCategoryFilter,G.UseProductFilter,G.IsActive,G.UseSizeFilter,G.UseQtyFilter,G.FilterSize,G.FilterQty,p1.ProductCode as MustProductCode, p2.ProductCode as GiftProductCode, B.Name as BName,L.Name as LName, ISNULL(PC.Name,'') as CName, ISNULL(PS.Name,'') as SCName  from GiftSystem as G left join Product as P1 on P1.Id = G.MustIncludeProductId left join Product as P2 on p2.Id = G.GiftProductId left join Brand as B on B.Id = G.FilterBrandId left join Line as L on L.Id = G.FilterLineId left join ProductCategory as PC on G.FilterCategoryId = PC.Id left join ProductSubCategory as PS on G.FilterSubCategoryId = PS.Id";
            ...
            string SqlNoveltySystem = "Select N.*, B.Name as BName,L.Name as LName from NoveltySystem  as N left join Brand as B on N.BrandId = B.Id left join Line as L on N.LineId = L.Id";
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure that you want to import the data?", "", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
            {
               ...
               // DecryptFile(fileName, destFileName);
                ...
                DataTable dtxmlLine = dsxml.Tables["Line"];
                ...
                if (dtxmlLine != null)
                {
                    label1.Text = "Step 2 of 32 : Processing Line table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlBrand.Rows.Count;
                    Progressbar1.Value = 1;
                    ...
                    foreach (DataRow dataRowFromXML in dtxmlLine.Rows)
                    {
                        Progressbar1.PerformStep();
                        Boolean sameRowExist = false;
                        String Name = dataRowFromXML["Name"].ToString();

                        APP_Data.Line FoundLine = entity.Lines.Where(x => x.Name == Name).FirstOrDefault();
                        //same brand name found
                        if (FoundLine != null)
                        {
                            sameRowExist = true;
                        ...
                        if (sameRowExist)
                        {
                            GetLineFromXML(dataRowFromXML, FoundLine);
                            entity.SaveChanges();

                            String oldLineId = dataRowFromXML["Id"].ToString();
                            String LName = dataRowFromXML["Name"].ToString();
                            //update in brand DataTables
                            if (dtxmlProduct != null && dtxmlProduct.Columns.Contains("LineId"))
                            {
                                foreach (DataRow o in dtxmlProduct.Select("LineId = '" + oldLineId + "' and LName ='" + LName.Replace("'", "''") + "'"))
                                {
                                    o["LineId"] = FoundLine.Id.ToString();
                                }
                            }
                            if (dtxmlGiftSystem != null && dtxmlGiftSystem.Columns.Contains("FilterLineId") && dtxmlGiftSystem.Columns.Contains("LName"))
                            {
                                foreach (DataRow o in dtxmlGiftSystem.Select("FilterLineId = '" + oldLineId + "' and LName ='" + LName.Replace("'", "''") + "'"))
                                {
                                    o["FilterLineId"] = FoundLine.Id.ToString();
                                }
                            }

                            if (dtxmlNoveltySystem != null && dtxmlNoveltySystem.Columns.Contains("LineId") && dtxmlNoveltySystem.Columns.Contains("LName"))
                            {
                                foreach (DataRow o in dtxmlNoveltySystem.Select("LineId = '" + oldLineId + "' and LName ='" + LName.Replace("'", "''") + "'"))
                                {
                                    o["LineId"] = FoundLine.Id.ToString();
                                }
                            }
                        ...
                        {
                            APP_Data.Line line = new APP_Data.Line();
                            GetLineFromXML(dataRowFromXML, line);
                            ...
                            if (dtxmlGiftSystem != null)
                            {
                                foreach (DataRow o in dtxmlGiftSystem.Select("FilterLineId = '" + oldLineID + "' and LName ='" + LName.Replace("'", "''") + "'"))
                                {
                                    o["FilterLineId"] = line.Id.ToString();
                                }
                            }
                            if (dtxmlNoveltySystem != null)
                            {
                ...
                if (dtxmlCity != null)
                {
                    ...
                    label1.Text = "Step 3 of 32 : Processing City table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 4 of 32 : Processing PointDeductionPercentage_History table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 5 of 32 : Processing VIP Member Rule table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlCustomer != null)
                {
                    ...
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    label1.Text = "Step 6 of 32 : Processing Customer table please wait!";
                    label1.Refresh();
                    IQueryable<Customer> iCustomer = entity.Customers;
                    foreach (DataRow dataRowFromXML in dtxmlCustomer.Rows)
                    {
                        ...
                        APP_Data.Customer FoundCustomer = iCustomer.Where(x => x.CustomerCode == CustomerCode).FirstOrDefault();
                ...
                {
                    label1.Text = "Step 7 of 32 : Processing Counter table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlUserRole != null)
                {
                    ...
                    label1.Text = "Step 8 of 32 : Processing User Role table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlUser != null)
                {
                    label1.Text = "Step 9 of 32 : Processing User table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlUser.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<User> iuser = entity.Users;
                    ...
                    foreach (DataRow dataFromXML in dtxmlUser.Rows)
                    {
                        ...
                        APP_Data.User FoundUser = iuser.Where(x => x.Name == Name && x.UserRoleId == UserRoleId).FirstOrDefault();
                        //same User Role found
                        if (FoundUser != null)
                        {
                            SameRowExit = true;
                ...
                if (dtxmlLoc_PointRedeemHistory != null)
                {
                    label1.Text = "Step 10 of 32 : Processing Point Redeem History table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlLoc_PointRedeemHistory.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<Loc_PointRedeemHistory> LOCPointRedeem = entity.Loc_PointRedeemHistory;
                    ...
                    foreach (DataRow dataRowFrmXML in dtxmlLoc_PointRedeemHistory.Rows)
                    {
                        ...
                        APP_Data.Loc_PointRedeemHistory FoundLoc_PointRedeemHistory = LOCPointRedeem.Where(x => x.CustomerId == CustomerId && x.DateTime == DateTime && x.RedeemAmount == redeemAmt).FirstOrDefault();
                ...
                {//Loop through dataRow come from xml and check if the Loc_CustomerPoint is already exist or Loc_CustomerPoint is new one
                    label1.Text = "Step 11 of 32 : Processing Customer Point table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlLoc_CustomerPoint.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<Loc_CustomerPoint> ilocPOint = entity.Loc_CustomerPoint;
                    foreach (DataRow dataRowFrmXML in dtxmlLoc_CustomerPoint.Rows)
                    {
                        ...
                        APP_Data.Loc_CustomerPoint FoundCustomerPoint = ilocPOint.Where(x => x.CustomerId == customerId).FirstOrDefault();
                ...
                {
                    label1.Text = "Step 12 of 32 : Processing Consignment Counter table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmLUnit != null)
                {
                    label1.Text = "Step 13 of 32 : Processing Unit table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 14 of 32 : Processing Product Category table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlProudctSubCategory != null)
                {
                    label1.Text = "Step 15 of 32 : Processing Product Sub Category table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlTax != null)
                {
                    label1.Text = "Step 16 of 32 : Processing Tax table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlProduct != null)
                {
                    label1.Text = "Step 17 of 32 : Processing Product table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlShop != null)
                {
                    label1.Text = "Step 18 of 32 : Processing Shop table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 19 of 32 : Processing Currency table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 20 of 32 : Processing Payment Type table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlGiftCard != null)
                {
                    label1.Text = "Step 21 of 32 : Processing Gift Card table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlGiftCard.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<GiftCard> iGiftCard = entity.GiftCards;
                    ...
                    foreach (DataRow dataFromXML in dtxmlGiftCard.Rows)
                    {
                        ...
                        APP_Data.GiftCard FoundGiftCard = iGiftCard.Where(x => x.CardNumber == CardNumber && x.IsDeleted == false).FirstOrDefault();
                        //test
                ...
                if (dtxmlGiftSystem != null)
                {//through rowdata come from xml and check if the GiftCard is already existed or new one;
                    label1.Text = "Step 22 of 32 : Processing Gift System table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlGiftSystem.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<GiftSystem> iGiftSystem = entity.GiftSystems;
                    foreach (DataRow dataFromXML in dtxmlGiftSystem.Rows)
                    {
                        ...
                        //if (dataFromXML["FilterBrandId!= null)
                        //{
                        //    dataFromXML["FilterBrandId"] = GetBrandIdWithName(dataFromXML["BName"].ToString(), dtxmlBrand"]);
                        //}
                        //if (dataFromXML["FilterCategoryId!= null)
                        //{
                        //    dataFromXML["FilterCategoryId"] = GetPCateIdWithName(dataFromXML["CName"].ToString(), dtxmlProductCategory"]);
                        //}
                        //if (dataFromXML["FilterSubCategoryId!= null)
                        //{
                        //    dataFromXML["FilterSubCategoryId"] = GetSubPCateIdWithName(dataFromXML["SCName"].ToString(), dtxmlProductSubCategory"]);
                        //}
                        //if (dataFromXML["GiftProductCode!= null)
                        //{
                        //    dataFromXML["GiftProductId"] = GetProductIdWithProductCode(dataFromXML["GiftProductCode"].ToString(), dtxmlProduct"]);
                        //}

                        //if (dataFromXML["MustProductCode!= null)
                        //{
                        //    dataFromXML["MustIncludeProductId"] = GetProductIdWithProductCode(dataFromXML["MustProductCode"].ToString(), dtxmlProduct"]);
                        //}

                        APP_Data.GiftSystem FoundGiftSystem = iGiftSystem.Where(x => x.Name == Name).FirstOrDefault();
                ...
                IQueryable<Transaction> iTransactions = entity.Transactions;
                if (dtxmlTransaction != null)
                {
                    label1.Text = "Step 23 of 32 : Processing Transaction table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    ...
                    foreach (DataRow dataRowFromXML in dtxmlTransaction.Rows)
                    {
                        ...
                        APP_Data.Transaction FoundTransaction = iTransactions.Where(x => x.Id == Id).FirstOrDefault();
                ...
                {
                    //Loop through dataRow come from xml and check if the transaction is already exist or a new one
                    label1.Text = "Step 24 of 32 : Processing Transaction Detail table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    ...
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<TransactionDetail> iTransactionDetails = entity.TransactionDetails;
                    foreach (DataRow dataRowFromXML in dtxmlTransactionDetail.Rows)
                    {
                        ...
                        APP_Data.TransactionDetail FoundTransactionDetail = iTransactionDetails.Where(x => x.TransactionId == TransactionId && x.ProductId == ProductId && x.BatchNo == BatchNo).FirstOrDefault();
                        //Same TransactionId Found
                ...
                {
                    //Loop through dataRow come from xml and check if the transaction is already exist or a new one
                    label1.Text = "Step 25 of 32 : Processing Transaction Payment Detail table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    //Progressbar1.Maximum = dtxmlTransactionDetail.Rows.Count;
                    ...
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<TransactionPaymentDetail> iTransactionPaymentDetail = entity.TransactionPaymentDetails;
                    foreach (DataRow dataRowFromXML in dtxmlTransactionPaymentDetail.Rows)
                    {
                        ...
                        //Boolean IsDelete = Convert.ToBoolean(dataRowFromXML["IsDeleted"].ToString());
                        //string ProductCode = dataRowFromXML["ProductCode"].ToString();
                        var FoundTransactionPaymentDetail = iTransactionPaymentDetail.Where(x => x.TransactionId == TransactionId && x.PaymentMethodId == paymentMethodId).FirstOrDefault();
                ...
                {//through rowdata come from xml and check if the GiftCardInTransaction is already existed or new one;
                    label1.Text = "Step 26 of 32 : Processing Gift Card In Transaction table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    Progressbar1.Maximum = dtxmlGiftCardInTransaction.Rows.Count;
                    Progressbar1.Value = 1;
                    Progressbar1.Step = 1;
                    IQueryable<GiftCardInTransaction> iGiftInTrans = entity.GiftCardInTransactions;
                    foreach (DataRow dataFromXML in dtxmlGiftCardInTransaction.Rows)
                    {
                        ...
                        APP_Data.GiftCardInTransaction FoundGiftCardInInTran = iGiftInTrans.Where(x => x.GiftCardId == GiftCardId && x.TransactionId == TransactionId).FirstOrDefault();
                ...
                if (dtxmlExchangeRateForTransaction != null)
                {
                    label1.Text = "Step 27 of 32 : Processing Exchange Rate For Transaction table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlDeleteLog != null)
                {
                    label1.Text = "Step 28 of 32 : Processing Delete Log table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                {
                    label1.Text = "Step 29 of 32 : Processing Attach Gift System For Transaction table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlUsePrePaidDebt != null)
                {
                    label1.Text = "Step 30 of 32 : Processing Use Pre Paid Debt table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                ...
                if (dtxmlNoveltySystem != null)
                {
                    label1.Text = "Step 31 of 32 : Processing Novelty System table please wait!";
                    label1.Refresh();
                    Progressbar1.Minimum = 1;
                    ...
                    foreach (DataRow dataFromXML in dtxmlNoveltySystem.Rows)
                    {
                        ...
                        //if (dataFromXML["BrandId!= null)
                        //{
                        //    dataFromXML["BrandId"] = GetBrandIdWithName(dataFromXML["BName"].ToString(), dtxmlBrand"]);
                        //}
                        ...
                        if (!SameRowExit)
                        {
                            ...
                            foreach (DataRow o in dtxmlProductInNovelty.Select("NoveltySystemId='" + OldNoveltySystemId + "'"))
                            {
                                ...
                                {
                                    //o["ProductId"] = GetProductIdWithProductCode(o["ProductCode"].ToString(), dtxmlProduct"]).ToString();
                        ...
                        else
                        {
                            ...
                            int NoveltySystemId = Convert.ToInt32(FoundNovelty.Id.ToString());//Convert.ToInt32(FoundNovelty.ProductInNovelties.Select(x => x.Id).FirstOrDefault());
                ...
                if (dtxmlSPDetail != null)
                {
                    ...
                    foreach (DataRow dataFromXML in dtxmlSPDetail.Rows)
                    {
                        ...
                        //if (dataFromXML["ParentProductCode!= null)
                        //{
                        //    dataFromXML["ParentProductID"] = GetProductIdWithProductCode(dataFromXML["ParentProductCode"].ToString(), dtxmlProduct"]).ToString();
                        //}
                        //if (dataFromXML["ChildProductCode!= null)
                        //{
                        //    dataFromXML["ChildProductID"] = GetProductIdWithProductCode(dataFromXML["ChildProductCode"].ToString(), dtxmlProduct"]).ToString();
                        //}
                        long ParentProductID = Convert.ToInt64(dataFromXML["ParentProductID"].ToString());

                        //if (dataFromXML["TransactionDetailID!= null)
                        //{
                        //    dataFromXML["TransactionDetailID"] = GetTransDetailIdWithProductIdAndTransaction(ParentProductID, dataFromXML["TID"].ToString()).ToString();
                        //}
        ...
        private void GetNoveltySystem(DataRow dataFromXML, APP_Data.NoveltySystem foundNoveltySytem, Boolean newRow)
        {
            if (foundNoveltySytem.UpdateDate < Convert.ToDateTime(dataFromXML["UpdateDate"].ToString()) || newRow == true)
            {
                ...
                if (dataFromXML.Table.Columns.Contains("LineId") && dataFromXML["LineId"].ToString().Trim() != string.Empty && dataFromXML["LineId"].ToString() != "")
                {
                    foundNoveltySytem.LineId = Convert.ToInt32(dataFromXML["LineId"].ToString());
                }
        ...
        private void GetGiftSystemFromXML(DataRow dataFromXML, APP_Data.GiftSystem FoundGiftSystem)
        {
            ...
            if (dataFromXML.Table.Columns.Contains("FilterLineId"))
            {
                if (dataFromXML["FilterLineId"].ToString() != "0" && dataFromXML["FilterLineId"].ToString() != "")
                {
                    FoundGiftSystem.FilterLineId = Convert.ToInt32(dataFromXML["FilterLineId"].ToString());
                }
            }
            ...
            if (dataFromXML.Table.Columns.Contains("UseLineFilter"))
            {
                if (!string.IsNullOrEmpty(dataFromXML["UseLineFilter"].ToString().Trim()))
                {
                    FoundGiftSystem.UseLineFilter = Convert.ToBoolean(dataFromXML["UseLineFilter"].ToString());
                }
            }
        }
        ...
        private void GetLineFromXML(DataRow dataRowFromXML, APP_Data.Line line)
        {
            line.Name = dataRowFromXML["Name"].ToString();
            line.LineCode = dataRowFromXML["LineCode"].ToString();
        }
        ...
        private void GetProductFromXML(DataRow dataRowFromXML, APP_Data.Product product)
        {
            ...
            if (dataRowFromXML.Table.Columns.Contains("LineId") && dataRowFromXML["LineId"].ToString() != "0" && dataRowFromXML["LineId"].ToString() != "")
            {
                product.LineId = Convert.ToInt32(dataRowFromXML["LineId"].ToString());
            }
        ...
        private void GetTransactionFromXML(DataRow dataRowFromXML, APP_Data.Transaction transaction)
        {
            ...
            //if (dataRowFromXML.Table.Columns.Contains("IsExported"))
            //{
            //    if (dataRowFromXML["IsExported"].ToString().Trim() != string.Empty && dataRowFromXML["IsExported"].ToString() != "")
            //    {
        ...
        //private long GetBrandIdWithName(string Name, DataTable tbBrand)
        //{
        //    if (string.IsNullOrEmpty(Name) || string.IsNullOrWhiteSpace(Name)) return 0;
        //    var iBrand = entity.Brands.Where(x => x.Name == Name).FirstOrDefault();
        //    if (iBrand != null)
        //    {
        //        return iBrand.Id;
        //    }
        //    else
        //    {
        //        DataRow iRow = tbBrand.Select("Name = '" + Name + "'").LastOrDefault();

        //        POSEntities entity2 = new POSEntities();
        //        APP_Data.Brand brand = new APP_Data.Brand();
        //        GetBrandFromXML(iRow, brand);

        //        entity2.Brands.Add(brand);
        //        entity2.SaveChanges();
        //        return brand.Id;
        //    }
        //}
        //private long GetPCateIdWithName(string Name, DataTable dtPCate)
        //{
        //    if (string.IsNullOrEmpty(Name) || string.IsNullOrWhiteSpace(Name)) return 0;
        //    var iPCategory = entity.ProductCategories.Where(x => x.Name == Name).FirstOrDefault();
        //    if (iPCategory != null)
        //    {
        //        return iPCategory.Id;
        //    }
        //    else
        //    {
        //        DataRow iRow = dtPCate.Select("Name = '" + Name + "'").LastOrDefault();

        //        POSEntities entity2 = new POSEntities();
        //        APP_Data.ProductCategory iPCate = new APP_Data.ProductCategory();
        //        GetProductCategoryFromXML(iRow, iPCate);

        //        entity2.ProductCategories.Add(iPCate);
        //        entity2.SaveChanges();
        //        return iPCate.Id;
        //    }
        //}
        //private long GetSubPCateIdWithName(string Name, DataTable tbSubP)
        //{
        //    if (string.IsNullOrEmpty(Name) || string.IsNullOrWhiteSpace(Name)) return 0;
        //    var iSubP = entity.ProductSubCategories.Where(x => x.Name == Name).FirstOrDefault();
        //    if (iSubP != null)
        //    {
        //        return iSubP.Id;
        //    }
        //    else
        //    {
        //        DataRow iRow = tbSubP.Select("Name = '" + Name + "'").LastOrDefault();
        //        POSEntities entity2 = new POSEntities();
        //        APP_Data.ProductSubCategory isubp = new APP_Data.ProductSubCategory();
        //        GetProductSubCategoryFromXML(iRow, isubp);

        //        entity2.ProductSubCategories.Add(isubp);
        //        entity2.SaveChanges();
        //        return isubp.Id;
        //    }
        //}
        //private long GetProductIdWithProductCode(string ProductCode, DataTable tbProduct)
        //{
        //    if (string.IsNullOrEmpty(ProductCode) || string.IsNullOrWhiteSpace(ProductCode)) return 0;
        //    var iProduct = entity.Products.Where(x => x.ProductCode == ProductCode).FirstOrDefault();
        //    if (iProduct != null)
        //    {
        //        return iProduct.Id;
        //    }
        //    else
        //    {
        //        DataRow iRow = tbProduct.Select("ProductCode = '" + ProductCode + "'").FirstOrDefault();
        //        POSEntities entity2 = new POSEntities();
        //        APP_Data.Product product = new APP_Data.Product();
        //        GetProductFromXML(iRow, product);
        //        product.Qty = 0;
        //        entity2.Products.Add(product);
        //        entity2.SaveChanges();
        //        return product.Id;
        //    }
        //}

        //private int GetTransDetailIdWithProductIdAndTransaction(long ProductId, string TransId)
        //{
        //    var TDID = entity.TransactionDetails.Where(x => x.ProductId == ProductId && x.TransactionId == TransId).FirstOrDefault();
        //    return TDID == null ? 0 : (int)TDID.Id;
        //}
        private void GetTransactionDetailFromXML(DataRow dataRowFromXML, APP_Data.TransactionDetail transactionDetail)
        {
            ...
            if (dataRowFromXML.Table.Columns.Contains("BatchNo"))
            {
                if (dataRowFromXML["BatchNo"].ToString().Trim() != string.Empty && dataRowFromXML["BatchNo"].ToString() != "")
                {
            ...
            {
                if (dataRowFromXML["BdDiscounted"].ToString().Trim() != string.Empty && dataRowFromXML["BdDiscounted"].ToString() != "")
                {
                    bool boolRes = false;
                    bool.TryParse(dataRowFromXML["BdDiscounted"].ToString(), out boolRes);
                    transactionDetail.BdDiscounted = boolRes;
                }
            }
        }
        ...
        private void GetCustomerFromXML(DataRow dataRowFromXML, APP_Data.Customer customer)
        {
            ...
            else if (dataRowFromXML["CustomerTypeId"] == null || dataRowFromXML["CustomerTypeId"].ToString() == null || string.IsNullOrEmpty(dataRowFromXML["CustomerTypeId"].ToString().Trim()))
            {
            ...
            {
                if (dataRowFromXML["PromoteDate"] == null || string.IsNullOrEmpty(dataRowFromXML["PromoteDate"].ToString()))
                {
