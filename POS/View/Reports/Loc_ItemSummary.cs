   ...
   && ((counterId != 0 && t.CounterId == counterId) || (counterId == 0 && 1 == 1))
            ...
            if (IsSale == true)
            {
                if (IsCash)
                {
                   ...
                   //TTN // CashTotal = (long)transList.Where(x => x.PaymentType.Name == "Cash").Sum(x => x.TotalAmount);
                    ...
                    AllTranslist.AddRange(transList.Where(x => x.PaymentType.Name == "Cash").ToList());
                }
                if (IsCredit)
                {

                    CreditTotal += FinalResultList.Where(x => x.PaymentId == 2).Sum(x => x.totalAmount);
                    AllTranslist.AddRange(transList.Where(x => x.PaymentType.Name == "Credit").ToList());
                    CreditReceive += Convert.ToInt64(transList.Where(x => x.PaymentType.Name == "Credit").Sum(x => x.RecieveAmount));
                }
                if (IsGiftCard)
                {

                    GiftCardAmount += (long)transList.Where(x => x.PaymentType.Name == "Gift Card").Sum(x => x.TotalAmount);
                    AllTranslist.AddRange(transList.Where(x => x.PaymentType.Name == "Gift Card").ToList());

                    List<Transaction> GTransList = transList.Where(x => x.PaymentType.Name == "Gift Card").ToList();
                ...
                if (IsMPU)
                {
                    // MPUAmount = (long)transList.Where(x => x.PaymentType.Name == "MPU Card").Sum(x => x.RecieveAmount);
                    MPUAmount += (from t in entity.TransactionPaymentDetails
                                  join td in entity.Transactions on t.TransactionId equals td.Id
                                  join p in entity.PaymentMethods on t.PaymentMethodId equals p.Id
                                  where EntityFunctions.TruncateTime((DateTime)td.DateTime) >= fromDate && EntityFunctions.TruncateTime((DateTime)td.DateTime) <= toDate
                                  && td.IsComplete == true && td.IsActive == true
                                  && td.IsDeleted == false && p.PaymentTypeId == 3 && ((counterId != 0 && td.CounterId == counterId) || (counterId == 0 && 1 == 1))
                                  select t.Amount).ToList().Sum();
                ...
                globalCard = (int)(from t in entity.TransactionPaymentDetails
                                   ...
                                   where EntityFunctions.TruncateTime((DateTime)td.DateTime) >= fromDate && EntityFunctions.TruncateTime((DateTime)td.DateTime) <= toDate
                                   && td.IsComplete == true && td.IsActive == true
                                   && td.IsDeleted == false && (p.PaymentTypeId == 4 || p.PaymentTypeId == 9 || p.PaymentTypeId == 10) && ((counterId != 0 && td.CounterId == counterId) || (counterId == 0 && 1 == 1))
                                   select t.Amount).ToList().Sum();
                bankTransfer = (int)(from t in entity.TransactionPaymentDetails
                                     ...
                                     where EntityFunctions.TruncateTime((DateTime)td.DateTime) >= fromDate && EntityFunctions.TruncateTime((DateTime)td.DateTime) <= toDate
                                     && td.IsComplete == true && td.IsActive == true
                                     && td.IsDeleted == false && p.PaymentTypeId == 5 && ((counterId != 0 && td.CounterId == counterId) || (counterId == 0 && 1 == 1))
                                     select t.Amount).ToList().Sum();
                pay = (int)(from t in entity.TransactionPaymentDetails
                            ...
                            where EntityFunctions.TruncateTime((DateTime)td.DateTime) >= fromDate && EntityFunctions.TruncateTime((DateTime)td.DateTime) <= toDate
                            && td.IsComplete == true && td.IsActive == true
                            && td.IsDeleted == false && p.PaymentTypeId == 6 && ((counterId != 0 && td.CounterId == counterId) || (counterId == 0 && 1 == 1))
                            select t.Amount).ToList().Sum();
        ...
        private void ShowReportViewer()
        {
           ...
           // CashTotal += GiftCardAmount;
            ...
            foreach (Transaction t in AllTranslist)
            {
                ...
                decimal itemDis = 0;
                if (t.PaymentType.Name != "Gift Card") // != 5)
                {
           ...
           // decimal actualAmount = Total;
            ...
            ReportParameter CashAmount = new ReportParameter("CashAmount", CashTotal.ToString()); //(CashTotal + CreditReceive).ToString());
            ...
            ReportParameter UsedGiftAmount = new ReportParameter("UsedGiftAmount", (newPaymentGAmount).ToString()); //(UseGiftAmount + newPaymentGAmount).ToString());
            ...
            ReportParameter ActualAmount = new ReportParameter("ActualAmount", CashTotal.ToString()); // actualAmount.ToString());
