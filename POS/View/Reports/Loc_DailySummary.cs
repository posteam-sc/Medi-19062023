...
namespace POS
{
    public partial class Loc_DailySummary : Form
    {
        #region Variable
        ...
        #region Event
        public Loc_DailySummary()
        {
            InitializeComponent();
        }

        private void Loc_DailySummary_Load(object sender, EventArgs e)
        {
            LoadData();
        ...
        private void LoadData()
        {
            ...
            if (IsSale == true)
            {
                ...
                if (IsGiftCard)
                {
                    itemList.AddRange(FinalResultList.Where(x => x.PaymentId == 8).ToList()); // 3 to 8 TTN
                    GiftCardAmount += FinalResultList.Where(x => x.PaymentId == 8).Sum(x => x.totalAmount);
                    AllTranslist.AddRange(transList.Where(x => x.PaymentTypeId == 8).ToList());

                    List<Transaction> GTransList = transList.Where(x => x.PaymentTypeId == 8).ToList();
                ...
                if (IsFOC)
                {
                    itemList.AddRange(FinalResultList.Where(x => x.PaymentId == 12).ToList()); //4 to 12
                ...
                if (IsMPU)
                {
                    itemList.AddRange(FinalResultList.Where(x => x.PaymentId == 3).ToList()); // 5 to 3
                    MPUAmount += FinalResultList.Where(x => x.PaymentId == 3).Sum(x => x.totalAmount);
                    AllTranslist.AddRange(transList.Where(x => x.PaymentTypeId == 3).ToList());
                }
                if (IsTester)
                {
                    itemList.AddRange(FinalResultList.Where(x => x.PaymentId == 11).ToList());// 6 to 11
