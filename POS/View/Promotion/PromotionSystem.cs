...
namespace POS
{
    public partial class PromotionSystem : Form
    {
        ...
        List<AttachGiftSystemForTransaction> attachList = new List<AttachGiftSystemForTransaction>();
        #endregion
        ...
        private void PromotionSystem_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                attachList = entity.AttachGiftSystemForTransactions.Where(x => x.AttachGiftSystemId == currentPromotion.Id).ToList();
                ...
                if (currentPromotion.UseLineFilter == true)
                {
                    if (currentPromotion.Line == null)
                    {
                        cboLine.Text = "None";
                    }
                    else
                    {
                        cboLine.Text = currentPromotion.Line.Name;
                    }
                }
        ...
        private void cboLine_SelectedValueChanged(object sender, EventArgs e)
        {
            if (cboLine.SelectedIndex > 1)
            {
                ...
                productList1.AddRange(entity.Products.Where(x => x.LineId == lineId).ToList());
                cboProduct.DataSource = productList1;
                cboProduct.DisplayMember = "Name";
                cboProduct.ValueMember = "Id";
                cboProduct.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            ...
            else if (cboLine.SelectedIndex == 1)
            {
                ...
                productList1.AddRange(entity.Products.Where(x => x.LineId == null).ToList());
                cboProduct.DataSource = productList1;
                cboProduct.DisplayMember = "Name";
                cboProduct.ValueMember = "Id";
                cboProduct.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            ...
            if (!hasError)
            {
                ...
                if (!isEdit)
                {
                    ...
                    if (data_gs == null)
                    {
                        ...
                        if (cboLine.SelectedIndex > 1)
                        {
                            ...
                            giftSystem.FilterLineId = Convert.ToInt32(cboLine.SelectedValue);
                            giftSystem.UseLineFilter = true;
                        }
                        else if (cboLine.SelectedIndex == 1)
                        {
                            giftSystem.UseLineFilter = true;
                        ...
                        {
                            giftSystem.UseLineFilter = false;
                        }
                        //Segment
                        if (cboSegment.SelectedIndex > 0)
                        {
                ...
                else
                {
                    ...
                    if (result.Equals(DialogResult.OK))
                    {
                        ...
                        if (cboLine.SelectedIndex > 1)
                        {
                            giftSystemObj.FilterLineId = Convert.ToInt32(cboLine.SelectedValue);
                            giftSystemObj.UseLineFilter = true;
                        }
                        else if (cboLine.SelectedIndex == 1)
                        {
                            giftSystemObj.UseLineFilter = true;
                        ...
                        {
                            giftSystemObj.UseLineFilter = false;
                        }
                        //Segment
