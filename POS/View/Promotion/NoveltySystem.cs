...
namespace POS
{
    public partial class NoveltySystem : Form
    {
        ...
        private void NoveltySystem_Load(object sender, EventArgs e)
        {
            ...
            APP_Data.Line lineObj = new APP_Data.Line();
            lineObj.Name = "Select";
            lineObj.Id = 0;
            lineList.Add(lineObj);
            lineList.AddRange(entity.Lines.ToList());
            ...
            if (isEdit)
            {
                ...
                cboLine.Text = noveltySysObj.Line.Name;
                List<Product> productList = entity.Products.Where(x => x.LineId == noveltySysObj.LineId).ToList();
        ...
        private void cboLine_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cboLine.SelectedIndex > 0)
            {
                ...
                List<Product> productList = entity.Products.Where(x => x.LineId == LineId).ToList();
                cboProduct.DataSource = productList;
                cboProduct.DisplayMember = "Name";
                cboProduct.ValueMember = "Id";
                tlpProduct.Enabled = true;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!hasError)
            {
                ...
                {
                    //string Key = null;
                    //int count = (from en in entity.NoveltySystems select en).Count();
                    //String dt = DateTime.Now.Date.ToString("dd-MMM-yyyy");
                    //string[] str = null;
                    //char[] splitspe = { '-' };
                    //str = dt.Split(splitspe);
                    //Key = "NV" + str[1] + str[2] + count.ToString();
                    ...
                    noveltyObj.LineId = Convert.ToInt32(cboLine.SelectedValue);

                    //noveltyObj.NovSysId = Key;
                ...
                else
                {
                    ...
                    if (IsContinue)
                    {
                        ...
                        if (result.Equals(DialogResult.OK))
                        {
                            ...
                            //  editObj.ProductInNovelties.Clear();

                            editObj.ValidFrom = dtpFrom.Value.Date;
                            editObj.ValidTo = dtpTo.Value.Date;
                            editObj.LineId = Convert.ToInt32(cboLine.SelectedValue);
