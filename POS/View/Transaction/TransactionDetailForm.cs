    ...
    public partial class TransactionDetailForm : Form
    {
        ...
        private void dgvTransactionDetail_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            foreach (DataGridViewRow row in dgvTransactionDetail.Rows)
            {
                ...
                row.Cells[ColItemName.Index].Value = transactionDetailObj.Product.Name;
                ...
                row.Cells[ColDPR.Index].Value = transactionDetailObj.IsDeductedBy == (decimal?)null ? "0.00%" : transactionDetailObj.IsDeductedBy + "%";
            }
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (transactionObj.PaymentType.Name == "Credit")
            {
                ...
                foreach (TransactionDetail transaction in PrintDetailList(transactionObj.TransactionDetails.ToList()))
                {
            ...
            else if (false)// (transactionObj.PaymentType.Name == "Gift Card")
            {
            ...
            else if (false)// (transactionObj.PaymentType.Name == "FOC" || transactionObj.PaymentType.Name == "Tester")
            {
            ...
            else if (false) //(transactionObj.PaymentType.Name == "MPU")
            {
                ...
                ReportParameter PaidAmount = new ReportParameter("PaidAmount", Convert.ToInt32(transactionObj.TotalAmount).ToString());
                rv.LocalReport.SetParameters(PaidAmount);
            ...
            else
            {
                ...
                foreach (var item in data)
                {
                    ...
                    if (newRow.PaymentMethod.Contains("Gift"))
                    {
                        GiftAmt += item.Amount;
                    }
                ...
                ReportParameter PaidAmount = new ReportParameter("PaidAmount", (Convert.ToInt32(transactionObj.TotalAmount) - GiftAmt).ToString());
                rv.LocalReport.SetParameters(PaidAmount);
        ...
        private void dgvTransactionDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                int currentTransactionId = Convert.ToInt32(dgvTransactionDetail.Rows[e.RowIndex].Cells[ColId.Index].Value.ToString());
                ...
                if (e.ColumnIndex == ColDelete.Index)
                {
                    ...
                    if (result.Equals(DialogResult.OK))
                    {
                        ...
                        {
                            bool res = Utility.CanRemoveSaleTransaction(transactionObject,tdOBj.TotalAmount.ToString(), (int)transactionObject.CustomerId, transactionObject.Customer.Name);
                            if (res == false)
                            {
                                MessageBox.Show("You can't delete this transaction, the points achieved for this transaction have been used or redeemed!", "Invalid", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            ...
                            if (dgvTransactionDetail.Rows.Count <= 1)
                            {
                                ...
                                if (result2.Equals(DialogResult.OK))
                                {
                                    ...
                                    Utility.AddProductAvailableQty(entity, (long)tds.ProductId, tds.BatchNo, (int)tds.Qty);
        ...
        private void LoadData()
        {
            ...
            else if (transactionObject.Type == TransactionType.Sale || transactionObject.Type == TransactionType.Credit)
            {
                ...
                //GiftCard
                else if (transactionObject.PaymentTypeId == 8)
                {
                    lblRecieveAmunt.Text = transactionObject.RecieveAmount.ToString();
