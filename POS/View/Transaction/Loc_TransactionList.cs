...
namespace POS
{
    public partial class Loc_TransactionList : Form
    {
        ...
        private void dgvTransactionList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                bool isexp = false;
                if (dgvTransactionList.Rows[e.RowIndex].Cells[dgvTransactionList.ColumnCount - 1].Value != null)
                {
                    bool.TryParse(dgvTransactionList.Rows[e.RowIndex].Cells[dgvTransactionList.ColumnCount - 1].Value.ToString(), out isexp);
                };
