...
namespace POS
{
    public partial class Login : Form
    {
        ...
        bool DisabledImportExport;
        public static bool IsBackDateExportSuccess { get; set; }
        public static bool IsAutoImportSuccess { get; set; }
        #endregion
        ...
        private void Login_Load(object sender, EventArgs e)
        {
            ...
            #region BackDateExport And AutoImport            

            IsBackDateExportSuccess = true;
            IsAutoImportSuccess = true;

            DisabledImportExport = SettingController.DisabledImportExport;
            if (!DisabledImportExport)
            {
                DataExport exportForm = new DataExport(null);
                exportForm.IsAutoExport = true;
                exportForm.IsBackDateExport = true;
                exportForm.ShowDialog();

                if (!IsBackDateExportSuccess)
                {
                    IsAutoImportSuccess = false;
                    return;
                }
                else
                {
                    ...
                    List<ImportExportLog> todayImLogList = entity.ImportExportLogs.Where(x => EntityFunctions.TruncateTime(x.ProcessingDateTime) == todayDate && x.Type == "Import").ToList();
                    
                    if (todayImLogList.Count == 0)
                    {
                        DataImport importForm = new DataImport(null);
                        importForm.IsAllDataImport = false;
                        importForm.IsAutoImport = true;
                        importForm.Text = "Auto Data Import";
                        importForm.ShowDialog();

                        if (!DataImport.IsImportSuccess)
                        {
                            IsAutoImportSuccess = false;
                            return;
                        }
                    }
                    else if (todayImLogList.Count == 1)
                    {
                        ImportExportLog todayimportlog = todayImLogList.FirstOrDefault();
                        if (todayimportlog.Status == "Pending" || todayimportlog.Status == "Fail")
                        {
                            DataImport importForm = new DataImport(null);
                            importForm.IsAllDataImport = false;
                            importForm.IsAutoImport = true;
        ...
        {
            //txtPassword.Text = "Sourcec0de";
            //txtUserName.Text = "sourcecodeadmin";
            //cboCounter.SelectedIndex = cboCounter.Items.Count -1;
            ...
            if (!hasError)
            {
                ...
                if (user != null)
                {
                    ...
                    if (p == password)
                    {
                        ...
                        if (DisabledImportExport)
                        {
                            ((MDIParent)this.ParentForm).importDataToolStripMenuItem.Visible = 
                            ((MDIParent)this.ParentForm).importDataByProductToolStripMenuItem.Visible = 
                            ((MDIParent)this.ParentForm).importAllDataToolStripMenuItem.Visible =
                            ((MDIParent)this.ParentForm).backDateExportToolStripMenuItem.Visible = 
                            ((MDIParent)this.ParentForm).exportDataToolStripMenuItem.Visible = 
                            ((MDIParent)this.ParentForm).refreshAccessTokenToolStripMenuItem.Visible = false;

                        }

                        //if (IsBackDateExportSuccess)
                        //{
                        //    ((MDIParent)this.ParentForm).backDateExportToolStripMenuItem.Visible = true;
                        //}
                        ...
                        form.IsBackDateExportSuccess = IsBackDateExportSuccess;
                        form.IsAutoImportSuccess = IsAutoImportSuccess;
                        form.Show();
