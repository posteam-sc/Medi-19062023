    ...
    public partial class PaidByCreditWithPrePaidDebt : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (!hasError)
            {
                ...
                {
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 1, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + ExtraTax - ExtraDiscount, totalCost, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                    ...
                    if (totalAmount != 0)
                    {
                        ...
                        if (tList.Count > 0)
                        {
                            ...
                            if (DebtAmount > 0)
                            {
                                System.Data.Objects.ObjectResult<string> DebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Settlement, true, true, 1, 0, 0, DebtAmount, DebtAmount, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                                entity.SaveChanges();
                            }
                        }
                        else
                    ...
                    if (receiveAmount > 0)
                    {
                        System.Data.Objects.ObjectResult<string> PreDebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Prepaid, true, false, 1, 0, 0, receiveAmount, receiveAmount, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                        entity.SaveChanges();
                    }
                }
                #endregion
                ...
                else
                {
                    if (totalAmount >= totalCost)
                    {
                        ...
                        {
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, true, true, 2, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + ExtraTax - ExtraDiscount, receiveAmount, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                            totalAmount -= receiveAmount;
                    ...
                    else
                    {
                        if (chkIsPrePaid.Checked == true)
                        {
                            ...
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, false, true, 2, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + ExtraTax - ExtraDiscount, receiveAmount, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                            totalAmount -= receiveAmount;
                            totalCost -= receiveAmount;
                            resultId = Id.FirstOrDefault().ToString();
                            insertedTransaction = (from trans in entity.Transactions where trans.Id == resultId select trans).FirstOrDefault<Transaction>();
                        ...
                        {
                            Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Credit, false, true, 2, ExtraTax + Tax, ExtraDiscount + Discount, DetailList.Sum(x => x.TotalAmount) + ExtraTax - ExtraDiscount, totalAmount, null, customerId, SettingController.DefaultShop.ShortCode, SettingController.DefaultShop.Id, false);
                            resultId = Id.FirstOrDefault().ToString();
                            insertedTransaction = (from trans in entity.Transactions where trans.Id == resultId select trans).FirstOrDefault<Transaction>();
