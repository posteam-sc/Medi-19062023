    ...
    public partial class Loc_CustomerDetail : Form
    {
        ...
        private void CustomerDetail_Load(object sender, EventArgs e)
        {
            ...
            if (plist.Count > 0)
            {
                ...
                if (DateTime.Now.Year == 2024 && DateTime.Now.Month < 6) // Updated by Lele on 2024-Mar-11, extend Pts expired date from 2023-Apr-01 to 2024-May-31 
                {
                    foreach (Loc_PointRedeemHistory po in plist)
                    {
                        ...
                        if (po.DateTime.Year == (DateTime.Now.Year) && po.DateTime.Month < 6) // Updated by Lele 2024-Mar-11, extend Pts expired date from 2023-Apr-01 to 2024-May-31
                        {
                            currentRedeemPoint += po.RedeemPoint;
                        }
                    }
        ...
        private void dgvVIPTransaction_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            foreach (DataGridViewRow row in dgvVIPTransaction.Rows)
            {
                ...
                // row.Cells[dgvVIPTransaction.ColumnCount - 1].Value = ts.IsExported;
            }
        }
        ...
        private void dgvVIPTransaction_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == 7)
                {
                    ...
                    {
                        bool isexp = false;
                        if (dgvVIPTransaction.Rows[e.RowIndex].Cells[dgvVIPTransaction.ColumnCount - 1].Value != null)
                        {
                            bool.TryParse(dgvVIPTransaction.Rows[e.RowIndex].Cells[dgvVIPTransaction.ColumnCount - 1].Value.ToString(), out isexp);
                        }
