...
namespace POS
{
    class Utility
    {
        ...
        public static bool CanRemoveSaleTransaction(Transaction currentTrans, string totalAmount, int CustomerId, string CustomerName)
        {

            int tamt = 0;
            if (!currentTrans.Loc_IsCalculatePoint)
            {
                return true;
            }
            if (totalAmount != null)
            {
                int.TryParse(totalAmount, out tamt);
            }

            if (CustomerName.ToLower() != "default")
            {
                int totalcustomerPoint = (int)Loc_CustomerPointSystem.GetPointFromCustomerId(CustomerId);
                int deletePoint = (int)tamt / 10000;
                if (totalcustomerPoint < deletePoint)
                {
                    //MessageBox.Show("There is no enough customer points to delete/refund.", "Can't Delete/Refund", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
            }
            return true;
        }
        public static void InsertRevokeHistoryData(Customer cus, int nonVIPId, string Note)
        {
            //using (TransactionScope transactionScope = new TransactionScope())
            //{
            //int LevelFrom = Convert.ToInt32(cus.CustomerTypeId);
            ...
            cus = pEntity.Customers.Find(cus.Id);
            cus.CustomerTypeId = nonVIPId;
        ...
        {
            if (ts != null && (ts.PaymentType.Name == "Gift Card" || ts.PaymentType.Name == "MultiPayment"))
            {
                POSEntities entity4GiftCard = new POSEntities();
                List<GiftCardInTransaction> gList = entity4GiftCard.GiftCardInTransactions.Where(x => x.TransactionId == ts.Id).ToList();
                foreach (GiftCardInTransaction gt in gList)
                {
                    if (gt != null && gt.GiftCard != null)
                    {
                        gt.GiftCard.IsUsed = false;

                    }
                }
                entity4GiftCard.SaveChanges();
            }
        }
    ...
    public static class Loc_CustomerPointSystem
    {
        public static int GetPointFromCustomerId(int CustomerId)
        {
            ...
            if (currentCustomer.CustomerTypeId == 1)
            {
                ...
                else if ((DateTime.Now.Year == 2023 && DateTime.Now.Month > 3) || (DateTime.Now.Year == 2024 && DateTime.Now.Month < 6))
                {
                ...
                {  //IsDeductedby must not be null and equal to Zero  then   The discount rate of product must be greater than the rate of Double Promothion RULE                        
                    if (Items.TransactionDetails != null)
                    {
    ...
    public class RoleManagementController
    {
        ...
        public RoleManagementModel SAP { get; set; }
    ...
    public static class SettingController
    {
        ...
        public static bool AdminShop
        {
            get
            {
                POSEntities entity = new POSEntities();
                APP_Data.Setting currentSet = entity.Settings.FirstOrDefault(x => x.Key == "IsAdminShop" && x.Value == "1");
                if (currentSet != null)
                {
                    return true;
                }
